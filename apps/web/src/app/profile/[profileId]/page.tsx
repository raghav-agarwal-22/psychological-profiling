...
export default function ProfilePage() {
  ...
  const [isPaidUser, setIsPaidUser] = useState(true) // optimistic: assume paid until we know otherwise
  ...
  useEffect(() => {
    ...
    // Fetch user info for subscription status + A/B test assignment
    api
      .get<{ user: { id: string; subscriptionTier: string } }>('/api/auth/me', token)
      .then((d) => {
        const paid = d.user.subscriptionTier !== 'free'
        setIsPaidUser(paid)
        if (!paid) {
          const variant = initABTest(d.user.id)
          setAbVariant(variant)
          posthog.capture('ab_test_view', { variant, section: 'profile_results' })
        }
      })
      .catch(() => {
        // Fallback: read stored variant if user fetch fails
        setAbVariant(getABVariant())
        setIsPaidUser(false)
      })

    api
      .get<{ profile: Profile; dimensionProgress?: DimensionProgressData }>(endpoint, token)
      .then((d) => {
        setProfile(d.profile)
        if (d.dimensionProgress) {
          setDimensionProgress(d.dimensionProgress)
          // Show unlock celebration if ?unlocked=1 query param is present
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-12">
      ...
      {/* New profile share nudge — shown once after completing an assessment */}
      {showNewProfileBanner && (
        <div className="mb-8 rounded-2xl border border-amber-500/20 bg-amber-500/5 p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="font-serif text-lg text-stone-100">Assessment complete</p>
              <p className="mt-1 text-sm text-stone-400">
                Share your result — every share brings someone new to Innermind.
              </p>
            </div>
            <button
              onClick={() => setShowNewProfileBanner(false)}
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            <button
              onClick={handleShare}
              disabled={sharing}
              className="flex items-center gap-2 rounded-xl bg-amber-500 px-5 py-2 text-sm font-semibold text-stone-950 transition-colors hover:bg-amber-400 disabled:opacity-50"
            >
      ...
      {/* Dimension unlock celebration toast */}
      {showUnlockCelebration && dimensionProgress && (
        <DimensionUnlockCelebration
          dimensionName={dimensionProgress.completed[dimensionProgress.completed.length - 1] ?? 'New dimension'}
          isComplete={dimensionProgress.count === dimensionProgress.total}
          onDismiss={() => setShowUnlockCelebration(false)}
        />
      )}

      {/* Dimensions progress bar */}
      {dimensionProgress && (
        <DimensionsProgress progress={dimensionProgress} className="mb-8" />
      )}
      ...
      {dimensionProgress && dimensionProgress.count < dimensionProgress.total && (() => {
        const LOCKED_META: Record<string, { name: string; headline: string; tease: string; duration: string }> = {
          JUNGIAN_ARCHETYPES: {
            name: 'Identity & Myth',
            headline: 'Your Archetypal Identity',
            tease: 'Discover which of 12 archetypes drives your worldview — and what lives in your shadow.',
            duration: '5–8 min',
          },
          ATTACHMENT_STYLE: {
            name: 'Relationship Blueprint',
            headline: 'How You Love and Connect',
            tease: 'Your attachment style shapes every close relationship. See the pattern.',
            duration: '3–5 min',
          },
          ENNEAGRAM: {
            name: 'Core Motivation',
            headline: "What You're Running Toward (and Away From)",
            tease: 'Your Enneagram type reveals the deep fear and desire beneath everything you do.',
            duration: '8–12 min',
          },
          VALUES_INVENTORY: {
            name: 'Purpose & Ethics',
            headline: "What You're Here to Protect",
            tease: "Your values are the non-negotiables. Find out which ones are truly yours.",
            duration: '5–8 min',
          },
        }
        const CORE_ORDER = ['JUNGIAN_ARCHETYPES', 'ATTACHMENT_STYLE', 'ENNEAGRAM', 'VALUES_INVENTORY']
        const locked = CORE_ORDER.filter((d) => !dimensionProgress.completed.includes(d))
        if (locked.length === 0) return null
        return (
          <div className="mb-8 space-y-4">
            <h2 className="font-serif text-xl text-stone-300">Deepen your portrait</h2>
            {locked.slice(0, 2).map((type) => {
              const meta = LOCKED_META[type]
              if (!meta) return null
              return (
                <LockedDimensionSection
                  key={type}
                  dimensionName={meta.name}
                  teaseHeadline={meta.headline}
                  teaseLine={meta.tease}
